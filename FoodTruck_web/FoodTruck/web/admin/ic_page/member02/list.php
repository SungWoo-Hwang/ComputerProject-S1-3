...
//아이디 검색
if ($search_id) {
	$que_where .= " and ( id like '%".$search_id."%'  ) ";
}

//업체상호명 검색
if ($search_name_sub) {
	$que_where .= " and ( name_sub like '%".$search_name_sub."%'  ) ";
}


//활동지역 검색
if ($search_place) {
	$que_where .= " and ( place01 like '%".$search_place."%'  )";
...
if ($search_phone_number) {
	$que_where .= " and ( phone_number like '%".$search_phone_number."%'  ) ";
}
$count_que = "select count(key_index) from mem_tb Where (1) AND SHOP_KEEPER = 'CUSTOMER' $que_where order by key_index desc";
...
-->
<form name="fmemberlist" id="fmemberlist" action="<?=$home_admin_proc_url?>/member_update2.php" method="post">
